  ...
  const buildSql = (telefoneExpr) => `
    WITH RESERVAS_BASE AS (
      SELECT
        ...
        RV.DATACHEGPREVISTA,
        RV.DATAPARTPREVISTA,
        TRUNC(TO_DATE('31/12/' || EXTRACT(YEAR FROM RV.DATACHEGPREVISTA), 'DD/MM/YYYY')) AS TARGET_DIA
      ...
      WHERE MH.PRINCIPAL = 'S'
        ...
        AND RV.IDHOTEL = ?
        AND TRUNC(TO_DATE('31/12/' || EXTRACT(YEAR FROM RV.DATACHEGPREVISTA), 'DD/MM/YYYY')) >= TRUNC(RV.DATACHEGPREVISTA)
        AND TRUNC(TO_DATE('31/12/' || EXTRACT(YEAR FROM RV.DATACHEGPREVISTA), 'DD/MM/YYYY')) < TRUNC(RV.DATAPARTPREVISTA)
    )
    SELECT
      R.IDRESERVASFRONT,
      R.NUMRESERVA,
  ...
  try {
    ...
    if (formato === 'excel') {
      ...
      return enviarCsv(
        ...
        csvRows,
        'relatorio-mesas-por-uh-anonovo.csv'
      );
    }
  ...
  try {
    ...
    if (formato === 'excel') {
      ...
      return enviarCsv(
        ...
        csvRows,
        'relatorio-uh-por-mesa-anonovo.csv'
      );
    }
    ...
    res.setHeader(
      'Content-Disposition',
      'inline; filename="relatorio-uh-por-mesa-anonovo.pdf"'
    );
  ...
  try {
    ...
    if (formato === 'excel') {
      ...
      return enviarCsv(
        ...
        csvRows,
        'relatorio-uhs-sem-marcacao-anonovo.csv'
      );
    }
