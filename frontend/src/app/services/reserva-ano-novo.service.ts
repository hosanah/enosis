...
export type ReservaAnoNovoFiltro = {
...
export class ReservaAnoNovoService {
  private readonly API = environment.apiUrl;

  constructor(private http: HttpClient) {}

  buscarReservas(
    filtro: ReservaAnoNovoFiltro
  ): Observable<{ data: any[]; total: number; page: number; size: number }> {
    ...
    return this.http.get<{ data: any[]; total: number; page: number; size: number }>(
      `${this.API}/anonovo/reservas`,
      { params }
    );
  }
  ...
  getReservasPorMesa(idmarcacaomesa: number) {
    ...
    }>(`${this.API}/anonovo/mesas/${idmarcacaomesa}/reservas`);
  }

  salvarMarcacao(payload: {
    idreservasfront: number;
    quantidade: number;
    idmarcacaomesa: number;
    observacao?: string;
  }) {
    return this.http.post<{ ok: boolean; atualizados: number; solicitados: number }>(
      `${this.API}/anonovo/marcacoes`,
      payload
    );
  }

  cancelarMarcacao(payload: { idreservasfront: number; idmarcacaomesa: number }) {
    return this.http.post<{ ok: boolean; afetados: number }>(
      `${this.API}/anonovo/marcacoes/cancelar`,
      payload
    );
  }
